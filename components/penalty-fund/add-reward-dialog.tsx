...
const AddRewardDialog: React.FC<AddRewardDialogProps> = ({
  ...
  const relatedChores = useMemo(() => {
    return recentChores.filter(
      (chore) =>
        chore.assigned_to === formData.userId && chore.status === 'completed'
    );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const amount = -Math.abs(parseFloat(formData.amount)); // Store as negative value
      const currentDate = new Date().toISOString();

      // Insert fund reward record (as negative amount in fund_penalties table)
      const { error: fundError } = await supabase
        .from('fund_penalties')
        .insert({
          household_id: householdId,
          user_id: formData.userId,
          amount: amount, // Negative value for rewards
      ...
      const { error: notificationError } = await supabase
        ...
        .insert({
          ...
          user_id: formData.userId,
          type: 'fund_reward',
          title: 'Household Fund Reward',
          message: `You've earned $${Math.abs(
            parseFloat(formData.amount)
          ).toFixed(2)} for: ${formData.reason}`,
          is_urgent: false,
          is_read: false,
        });
      ...
      console.error('Error adding fund reward:', error);
      showErrorAlert('Failed to add reward. Please try again.');
    } finally {
        ...
        <div className="animate-in fade-in-50 duration-200">
                    ...
                    {isLoading ? (
                          ...
                          {relatedChores.map((chore) => (
                            ...
                            >
                              <div className="flex items-center justify-between w-full min-w-0 gap-2">
                                ...
                                <Badge
                                  variant={
                                    getChoreStatusBadgeProps(chore.status || '')
                                      .variant
                                  }
                      ...
                      <div className="p-3 border-2 border-dashed border-gray-200 rounded-lg text-center">
                        <p className="text-sm text-gray-500">
                          No completed chores found for this member
                        </p>
                        <p className="text-xs text-gray-400 mt-1">
                          Completed chores assigned to the selected member will
                          appear here
                        </p>
                      </div>
                    )}
          ...
          <DialogFooter className="flex flex-col-reverse sm:flex-row gap-3 sm:gap-2 pt-6 sm:pt-4">
            ...
            <Button
              ...
              className="w-full sm:w-auto text-base sm:text-sm bg-green-600 hover:bg-green-700 hover:scale-105 active:scale-95 transition-transform"
            >
              {isSubmitting ? (
                <>
                ...
                <>
                  ...
                  Add ${formData.amount || '0.00'} Reward
                </>
              )}
            </Button>
          </DialogFooter>
